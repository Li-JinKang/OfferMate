...
/**
 * 分级记忆 tool 族：把简历记忆按需暴露给 AI，逐层继续加载（L1→L2→L3），避免一次灌入全部记忆。
 *
 * - L1 [ListMemoryProfilesTool]：有哪些方向记忆，供 AI 挑相关的
 * - L2 [LoadProfileOverviewTool]：某方向的技能 + 项目/经历概览（可 query 过滤）
 * - L3 [LoadProjectDetailTool] / [LoadExperienceDetailTool]：继续加载具体项目/经历全文
 *
...
class ListMemoryProfilesTool(private val store: MemoryStore) : Tool {
    override val spec = ToolSpec(
        ...
        description = "读取用户本人的简历记忆入口。这里存放着用户的**简历信息**——按求职方向组织" +
            "（如 Java 后端、Android），每份含技能、项目、工作/实习经历等。凡是涉及用户本人的简历、" +
    ...
    override suspend fun call(argumentsJson: String): String {
        ...
        if (profiles.isEmpty()) return "（用户尚无任何简历记忆）"
...
class LoadProfileOverviewTool(
    ...
    override val spec = ToolSpec(
        ...
        description = "加载用户某个求职方向的**简历概览**：技能清单 + 项目/经历简介，附带通用事实（年限/学历等）。" +
            "这是了解用户简历内容的主要入口。profileId 取自 list_memory_profiles。可传 query 关键词仅返回相关行。" +
            "若要点评/改进某个具体项目或经历，再用 load_project_detail / load_experience_detail 继续加载全文。",
        parametersJson = """
...
abstract class DetailTool(
    ...
    protected fun detailSpec() = ToolSpec(
        name = toolName,
        description = "加载用户简历里某个${label}的详细内容（背景/职责/技术栈/难点/亮点），用于深入点评或结合作答。" +
