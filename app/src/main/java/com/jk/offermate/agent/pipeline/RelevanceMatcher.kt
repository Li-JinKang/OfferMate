    ...
    fun buildMessages(questions: List<ExtractedQuestion>): List<ChatMessage> {
        ...
        return listOf(
            ChatMessage(
                role = Role.SYSTEM,
                content = """
                    你需要结合用户的简历背景，评估每道题与其求职方向/技能/项目的相关程度。
                    获取背景的方式（按需分级调用工具，不要臆造）：
                    1. 先调用 list_memory_profiles 查看用户有哪些求职方向记忆，选择与题目最相关的一份。
                    2. 用 load_profile_overview(profileId) 加载该方向的技能与项目/经历概览。
                    3. 如需某段项目/经历细节，再用 load_project_detail / load_experience_detail 继续加载。
                    若没有任何记忆，则仅按题目本身的通用性给出保守评分。
                    4. 你需要分清楚：所有求职方向都需要的**通用知识**、某个求职方向需要掌握的**特殊知识**，
                    比如：算法/数据结构（如笔试编程题、LeetCode 风格题）、操作系统、计算机网络
